...
def add_security_zone(host, access_token, zone_name):
    ...
    if response.status_code != 200:
        ...
        print("Successfully added a security zone.")
...
def post_deployment(host, access_token):
    ...
    if response.status_code != 200:
        ...
        print(name, ". Deployment will take a few minutes...")
...
def main():
    ...
    for item in security_zones["items"]:
        print("\t %12s %12s" %(item['name'], item['mode']))

    # Add a new security zone
    zone_name = input("  Enter the name of a new security zone: ")
